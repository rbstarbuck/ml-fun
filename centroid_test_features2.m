function centroid_test_features2()
...
%}
handle{1} = @dc_fft_rms_handle;
handle{2} = @abs_corr_coefficient_handle;
handle{3} = @abs_corr_handle;
handle{4} = @peak_to_peak_handle;
handle{5} = @abs_fft_peak_to_peak_handle;
handle{6} = @abs_average_peak_percentile_handle;
%{
...
handle{18} = @abs_mean_handle;
handle{19} = @(x)mean(abs(x));
handle{20} = @var;
handle{21} = @abs_fft_sum_handle;
handle{22} = @abs_fft_skewness_handle;

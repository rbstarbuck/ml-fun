function [trainData, trainLabels, testData, testLabels] = leave_k_out(subjectsToLeaveOut, numSubjects, windowSize, windowIncrement, featureHandle)
%% LEAVE_K_OUT
%  Partitions all available data into an action-segmented training set of k
%  subjects and a n-k subject test set which is not included in the training data
...
for subjectId = 1:numSubjects
    if ismember(subjectId, subjectsToLeaveOut)
        testData = [testData(:,:) ; eval(sprintf('subject%g_xyz', subjectId))];
        testLabels = [testLabels(:) ; eval(sprintf('subject%g_label', subjectId))];
    else

function centroid_test_features4()
...
%}
handle{1} = @(x)mean(abs(x));
handle{2} = @var;
handle{3} = @abs_fft_sum_handle;
handle{4} = @abs_fft_skewness_handle;
handle{5} = @abs_skewness_handle;
handle{6} = @abs_fft_kurtosis_handle;
